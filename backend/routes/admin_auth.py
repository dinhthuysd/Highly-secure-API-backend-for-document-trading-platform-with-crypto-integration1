...
async def admin_login(login_data: AdminLogin, request: Request, db = Depends(get_db)):
    ...
    """
    # Find admin user
    admin_user = await db.admin_users.find_one({"email": login_data.email})
...
async def get_admin_profile(
    ...
    admin = await db.admin_users.find_one({"id": current_admin['id']}, {"_id": 0, "password_hash": 0, "totp_secret": 0})
